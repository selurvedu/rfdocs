...
# This module is a part of django-bootstrap-pagination package
...
def get_page_url(page_num, current_app, url_view_name, url_extra_args, url_extra_kwargs,
                 url_param_name, url_get_params):
    """
    Helper function to return a valid URL string given the template tag parameters
    """
    if url_view_name is not None:
        # Add page param to the kwargs list.
        # Overrides any previously set parameter of the same name.
        url_extra_kwargs[url_param_name] = page_num

        # This bit of code comes from the default django url tag
        try:
            url = reverse(url_view_name, args=url_extra_args, kwargs=url_extra_kwargs,
...
class BootstrapPagerNode(Node):
    ...
    def render(self, context):
        ...
        if page.has_previous():
            previous_page_url = get_page_url(page.previous_page_number(), context.current_app,
                                             url_view_name, url_extra_args, url_extra_kwargs,
                                             url_param_name, url_get_params)

        next_page_url = None
        if page.has_next():
            next_page_url = get_page_url(page.next_page_number(), context.current_app,
                                         url_view_name, url_extra_args, url_extra_kwargs,
                                         url_param_name, url_get_params)
...
class BootstrapPaginationNode(Node):
    ...
    def render(self, context):
        ...
        if alignment not in ["left", "center", "right"]:
            raise Exception("Optional argument \"alignment\" expecting one "
        ...
        if current_page <= page_count:
            last_page_url = get_page_url(page_count,
                                         context.current_app,
                                         url_view_name,
                                         url_extra_args,
                                         url_extra_kwargs,
...
def bootstrap_paginate(parser, token):
    ...
    if len(bits):
        for bit in bits:
            ...
            if not match:
                raise TemplateSyntaxError("Malformed arguments to "
                                          "bootstrap_pagination paginate tag")
